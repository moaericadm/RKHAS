...
def create_app():
    ...
    # <<< بداية التعديل: إزالة منطق جدولة حاكم السوق من هنا >>>
    # تم نقل هذا المنطق إلى admin_api.py ليتم التحكم به ديناميكياً
    ...
    # تمرير كائن "app" لكل وظيفة باستخدام "args"
    ...
    # <<< بداية التعديل: إزالة جدولة حاكم السوق من هنا >>>
    # if not scheduler.get_job('market_justice_job'):
    #     scheduler.add_job(id='market_justice_job', func=scheduled_tasks.automated_market_balance, trigger='interval', seconds=total_seconds, args=[app])
    #     print(f">> Automated Market Justice System (SAM) job scheduled to run every {total_seconds} seconds.")
    # <<< نهاية التعديل >>>
