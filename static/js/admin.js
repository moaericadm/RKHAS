document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.error("Firebase initialization error in auth.js:", e);
        return;
    }

    if (typeof firebase.auth !== 'function') {
        console.error("Firebase Auth library not loaded or failed to load. Authentication is disabled.");
        const authModal = document.getElementById('authModal');
        if (authModal) {
            const content = authModal.querySelector('.custom-modal-content');
            if (content) {
                content.innerHTML = '<span class="custom-close-btn">×</span><h5 class="text-danger">خطأ في التحميل</h5><p>فشل تحميل وحدة المصادقة. قد يكون السبب مشكلة في الشبكة أو وجود إضافات تمنع تحميل السكربتات. الرجاء المحاولة مرة أخرى.</p>';
                content.querySelector('.custom-close-btn').addEventListener('click', () => authModal.classList.remove('show'));
            }
        }
        return;
    }
