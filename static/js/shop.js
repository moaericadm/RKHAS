
    // --- START OF FILE static/js/shop.js (WITH AVATAR SHOP LOGIC) ---

    let isDomReady = false;
let isFirebaseReady = false;
let allCrawlers = [];
let ownedAvatars = new Set(); // To keep track of purchased avatar IDs

function tryToStartApp() {
    if (isDomReady && isFirebaseReady) {
        initializeShopPage();
    }
}

document.addEventListener('DOMContentLoaded', () => { isDomReady = true; tryToStartApp(); });
document.addEventListener('firebase-ready', () => { isFirebaseReady = true; tryToStartApp(); });

function initializeShopPage() {
    const ui = {
        walletCcBalance: document.getElementById('wallet-cc-balance'),
        walletSpBalance: document.getElementById('wallet-sp-balance'),
        productsContainer: document.getElementById('products-container'),
        spinProductsContainer: document.getElementById('spin-products-container'),
        pointsProductsContainer: document.getElementById('points-products-container'),
        avatarProductsContainer: document.getElementById('avatar-products-container'), // New UI element
    };

    let db;
    let currentUserId;
    let currentWallet = { cc: 0, sp: 0 };

    async function initializeApp() {
        console.log("Shop page initialization started (triggered by event).");
        try {
            db = firebase.database();
            const token = sessionStorage.getItem('firebaseToken');

            if (token) {
                await firebase.auth().signInWithCustomToken(token);
                currentUserId = firebase.auth().currentUser.uid;
            } else {
                throw new Error('توكن المصادقة مفقود. لا يمكن تحميل بيانات المتجر.');
            }

            if (currentUserId) {
                const crawlersSnapshot = await db.ref('users').orderByChild('name').get();
                if (crawlersSnapshot.exists()) {
                    allCrawlers = Object.entries(crawlersSnapshot.val()).map(([key, value]) => ({ ...value, name: key }));
                }
                setupDataListeners();
            } else {
                handleAuthError(new Error("لا يمكن تحديد هوية المستخدم."));
            }
        } catch (e) {
            handleAuthError(e);
        }
    }

    function handleAuthError(e) {
        console.error("CRITICAL SHOP PAGE ERROR:", e.message);
        const errorHtml = `<div class="col-12 text-center py-5"><p class="text-danger">فشل تحميل بيانات المتجر.<br>السبب: ${e.message}<br>يرجى <a href="/auth/logout">تسجيل الخروج</a> والمحاولة مرة أخرى.</p></div>`;
        Object.values(ui).forEach(container => {
            if (container && container.id !== 'wallet-cc-balance' && container.id !== 'wallet-sp-balance') {
                container.innerHTML = errorHtml;
            }
        });
    }

    function setupDataListeners() {
        const handleFirebaseError = (error, path) => console.error(`Firebase Read Error at ${path}:`, error.code, error.message);

        if (currentUserId) {
            db.ref(`wallets/${currentUserId}`).on('value', (s) => renderWallet(s.val()), (e) => handleFirebaseError(e, 'wallet'));
            db.ref(`user_avatars/${currentUserId}/owned`).on('value', (s) => {
                ownedAvatars = new Set(Object.keys(s.val() || {}));
                db.ref('site_settings/shop_avatars').once('value', snapshot => renderAvatarProducts(snapshot.val()));
            }, (e) => handleFirebaseError(e, 'user_avatars'));
        }

        db.ref('site_settings/shop_products').on('value', (s) => { renderProducts(s.val()); }, (e) => handleFirebaseError(e, 'shop_products'));
        db.ref('site_settings/shop_products_spins').on('value', (s) => { renderSpinProducts(s.val()); }, (e) => handleFirebaseError(e, 'shop_products_spins'));
        db.ref('site_settings/shop_products_points').on('value', (s) => { renderPointsProducts(s.val()); }, (e) => handleFirebaseError(e, 'shop_products_points'));
        db.ref('site_settings/shop_avatars').on('value', (s) => { renderAvatarProducts(s.val()); }, (e) => handleFirebaseError(e, 'shop_avatars'));
    }

    const formatNumber = (num) => new Intl.NumberFormat('en-US').format(Math.round(num || 0));

    function renderWallet(wallet) {
        currentWallet = wallet || { cc: 0, sp: 0 };
        if (ui.walletCcBalance) ui.walletCcBalance.textContent = formatNumber(currentWallet.cc);
        if (ui.walletSpBalance) ui.walletSpBalance.textContent = formatNumber(currentWallet.sp);
    }

    function renderAvatarProducts(avatarsData) {
        if (!ui.avatarProductsContainer) return;
        const products = avatarsData ? Object.entries(avatarsData) : [];

        if (products.length === 0) {
            ui.avatarProductsContainer.innerHTML = `<div class="col-12 text-center py-5"><p class="text-muted">لا توجد أفاتارات متاحة في المتجر حالياً.</p></div>`;
            return;
        }

        ui.avatarProductsContainer.innerHTML = products.map(([id, p]) => {
            const isOwned = ownedAvatars.has(id);
            const personalPrice = p.price_sp_personal || 0;
            const giftPrice = p.price_sp_gift || 0;

            const buyButton = isOwned
                ? `<button class="btn btn-secondary w-100" disabled><i class="bi bi-check-circle-fill me-2"></i>تم الشراء</button>`
                : `<button class="btn btn-primary w-100 buy-avatar-btn" data-avatar-id="${id}" data-avatar-name="${p.name}" data-sp-price="${personalPrice}"><i class="bi bi-person-fill me-2"></i>شراء لنفسي</button>`;

            const giftButton = `<button class="btn btn-success w-100 gift-avatar-btn" data-avatar-id="${id}" data-avatar-name="${p.name}" data-sp-price="${giftPrice}"><i class="bi bi-gift-fill me-2"></i>طلب إهداء</button>`;

            return `
                <div class="col-md-6 col-lg-4">
                    <div class="card product-card text-center h-100">
                        <div class="card-body d-flex flex-column">
                            <img src="${p.image_url}" class="avatar-product-icon" alt="Avatar">
                            <h5 class="avatar-name mb-3">${p.name}</h5>
                            
                            <div class="d-flex justify-content-around mb-3">
                                <div>
                                    <span class="price-label">شراء شخصي</span>
                                    <p class="price-value mb-0">${formatNumber(personalPrice)} SP</p>
                                </div>
                                <div>
                                    <span class="price-label">إهداء لصديق</span>
                                    <p class="price-value mb-0">${formatNumber(giftPrice)} SP</p>
                                </div>
                            </div>

                            <div class="mt-auto d-grid gap-2">
                                ${buyButton}
                                ${giftButton}
                            </div>
                        </div>
                    </div>
                </div>
            `;
        }).join('');

        document.querySelectorAll('.buy-avatar-btn').forEach(b => b.addEventListener('click', handleAvatarPurchase));
        document.querySelectorAll('.gift-avatar-btn').forEach(b => b.addEventListener('click', handleAvatarGiftRequest));
    }


    function renderProducts(productsData) {
        if (!ui.productsContainer) return;
        const products = productsData ? Object.entries(productsData) : [];
        if (products.length === 0) {
            ui.productsContainer.innerHTML = `<div class="col-12 text-center py-5"><p class="text-muted">لا توجد حزم SP متاحة في المتجر حالياً.</p></div>`;
            return;
        }
        ui.productsContainer.innerHTML = products.map(([id, p]) => `<div class="col-md-6 col-lg-4"><div class="card product-card text-center h-100"><div class="card-body d-flex flex-column"><img src="data:image/png;base64," class="product-icon" alt="SP Icon"><h4 class="product-sp">${formatNumber(p.sp_amount)} SP</h4><div class="mt-auto"><p class="mb-2">السعر:</p><h5 class="product-price-cc">${formatNumber(p.cc_price)} CC <img src="data:image/png;base64," alt="CC Icon"></h5><div class="d-grid"><button class="btn btn-success buy-sp-btn" data-product-id="${id}" data-sp-amount="${p.sp_amount}" data-cc-price="${p.cc_price}"><i class="bi bi-cart-plus-fill me-2"></i>شراء</button></div></div></div></div></div>`).join('');
        document.querySelectorAll('.buy-sp-btn').forEach(b => b.addEventListener('click', handleProductPurchase));
    }
    function renderSpinProducts(productsData) {
        if (!ui.spinProductsContainer) return;
        const products = productsData ? Object.entries(productsData) : [];
        if (products.length === 0) {
            ui.spinProductsContainer.innerHTML = `<div class="col-12 text-center py-5"><p class="text-muted">لا توجد حزم محاولات متاحة حالياً.</p></div>`;
            return;
        }
        ui.spinProductsContainer.innerHTML = products.map(([id, p]) => `<div class="col-md-6 col-lg-4"><div class="card product-card text-center h-100"><div class="card-body d-flex flex-column"><i class="bi bi-arrow-repeat product-icon" style="font-size:5rem;color:var(--bs-info);margin:auto;"></i><h4 class="product-attempts">${formatNumber(p.attempts_amount)} محاولات</h4><div class="mt-auto"><p class="mb-2">السعر:</p><h5 class="product-price-sp">${formatNumber(p.sp_price)} SP <img src="data:image/png;base64," alt="SP Icon"></h5><div class="d-grid"><button class="btn btn-info buy-spin-btn" data-product-id="${id}" data-attempts-amount="${p.attempts_amount}" data-sp-price="${p.sp_price}"><i class="bi bi-cart-plus-fill me-2"></i>شراء</button></div></div></div></div></div>`).join('');
        document.querySelectorAll('.buy-spin-btn').forEach(b => b.addEventListener('click', handleSpinPurchase));
    }
    function renderPointsProducts(productsData) {
        if (!ui.pointsProductsContainer) return;
        const products = productsData ? Object.entries(productsData) : [];
        if (products.length === 0) {
            ui.pointsProductsContainer.innerHTML = `<div class="col-12 text-center py-5"><p class="text-muted">لا توجد منتجات لتعديل الأسهم حالياً.</p></div>`;
            return;
        }
        ui.pointsProductsContainer.innerHTML = products.map(([id, p]) => { const t = p.type === 'raise' ? 'رفع أسهم' : 'إسقاط أسهم', i = p.type === 'raise' ? 'bi-arrow-up-circle-fill text-success' : 'bi-arrow-down-circle-fill text-danger', o = p.type === 'raise' ? `+${formatNumber(p.points_amount)}` : `-${formatNumber(p.points_amount)}`; return `<div class="col-md-6 col-lg-4"><div class="card product-card text-center h-100"><div class="card-body d-flex flex-column"><i class="bi ${i} product-points-icon"></i><h5>${t}</h5><h4 class="fw-bold">${o} نقطة</h4><div class="mt-auto"><p class="mb-2">السعر:</p><h5 class="product-price-sp">${formatNumber(p.sp_price)} SP <img src="data:image/png;base64," alt="SP Icon"></h5><div class="d-grid"><button class="btn btn-info buy-points-product-btn" data-product-id="${id}" data-sp-price="${p.sp_price}" data-title="${t}" data-daily-limit="${p.daily_limit}"><i class="bi bi-cart-plus-fill me-2"></i>شراء</button></div></div></div></div></div>` }).join('');
        document.querySelectorAll('.buy-points-product-btn').forEach(b => b.addEventListener('click', handlePointsProductPurchase));
    }

    async function apiCall(endpoint, options) {
        const response = await fetch(endpoint, options);
        // <<< التصحيح هنا: التعامل مع الاستجابة غير الناجحة بشكل أفضل >>>
        if (!response.ok) {
            const errorText = await response.text();
            let errorMessage = `خطأ في الخادم (${response.status})`;
            try {
                // محاولة قراءة رسالة الخطأ كـ JSON
                const errorJson = JSON.parse(errorText);
                errorMessage = errorJson.message || errorMessage;
            } catch (e) {
                // إذا فشلت، اعرض النص الخام للخطأ (مفيد في تصحيح أخطاء HTML)
                console.error("Could not parse error response as JSON. Raw response:", errorText);
            }
            throw new Error(errorMessage);
        }
        return response.json();
    }

    function handleProductPurchase(e) { const btn = e.currentTarget; const { productId, spAmount, ccPrice } = btn.dataset; if (currentWallet.cc < parseFloat(ccPrice)) { Swal.fire('فشل!', 'رصيدك من CC غير كافٍ.', 'error'); return } Swal.fire({ title: 'تأكيد الشراء', html: `هل تريد شراء <b>${formatNumber(spAmount)} SP</b> مقابل <b>${formatNumber(ccPrice)} CC</b>؟`, icon: 'question', showCancelButton: true, confirmButtonColor: '#28a745', cancelButtonColor: '#d33', confirmButtonText: 'نعم، قم بالشراء!', cancelButtonText: 'إلغاء' }).then(async r => { if (r.isConfirmed) { const o = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span>`; try { const d = await apiCall('/api/shop/buy_product', { method: 'POST', body: new URLSearchParams({ product_id: productId }) }); Swal.fire('تمت العملية!', d.message, 'success') } catch (e) { Swal.fire('فشل!', e.message, 'error') } finally { btn.disabled = false; btn.innerHTML = o } } }); }
    function handleSpinPurchase(e) { const btn = e.currentTarget; const { productId, attemptsAmount, spPrice } = btn.dataset; if (currentWallet.sp < parseFloat(spPrice)) { Swal.fire('فشل!', 'رصيدك من SP غير كافٍ.', 'error'); return } Swal.fire({ title: 'تأكيد الشراء', html: `هل تريد شراء <b>${attemptsAmount} محاولات</b> مقابل <b>${formatNumber(spPrice)} SP</b>؟`, icon: 'question', showCancelButton: true, confirmButtonColor: '#17a2b8', cancelButtonColor: '#d33', confirmButtonText: 'نعم، قم بالشراء!', cancelButtonText: 'إلغاء' }).then(async r => { if (r.isConfirmed) { const o = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span>`; try { const d = await apiCall('/api/shop/buy_spin_attempt', { method: 'POST', body: new URLSearchParams({ product_id: productId }) }); Swal.fire('تمت العملية!', d.message, 'success') } catch (e) { Swal.fire('فشل!', e.message, 'error') } finally { btn.disabled = false; btn.innerHTML = o } } }); }
    async function handlePointsProductPurchase(e) { const btn = e.currentTarget; const { productId, spPrice, title } = btn.dataset; if (currentWallet.sp < parseFloat(spPrice)) { Swal.fire('فشل!', 'رصيدك من SP غير كافٍ.', 'error'); return } if (allCrawlers.length === 0) { Swal.fire('خطأ!', 'لا يوجد زواحف لاستهدافهم.', 'info'); return } const options = allCrawlers.map(c => `<option value="${c.name}">${c.name}</option>`).join(''); const { value: selected } = await Swal.fire({ title: `تأكيد: ${title}`, html: `<p>اختر الزاحف المستهدف. التكلفة: <b>${formatNumber(spPrice)} SP</b>.</p><select id="crawler-select" class="swal2-select">${options}</select>`, focusConfirm: false, showCancelButton: true, confirmButtonText: 'تأكيد الشراء', cancelButtonText: 'إلغاء', preConfirm: () => document.getElementById('crawler-select').value }); if (selected) { const o = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span>`; try { const d = await apiCall('/api/shop/buy_points_product', { method: 'POST', body: new URLSearchParams({ product_id: productId, target_crawler: selected }) }); Swal.fire('تمت العملية!', d.message, 'success') } catch (r) { Swal.fire('فشل!', r.message, 'error') } finally { btn.disabled = false; btn.innerHTML = o } } }

    function handleAvatarPurchase(e) {
        const btn = e.currentTarget;
        const { avatarId, avatarName, spPrice } = btn.dataset;

        if (currentWallet.sp < parseFloat(spPrice)) {
            Swal.fire('فشل!', 'رصيدك من SP غير كافٍ.', 'error');
            return;
        }

        Swal.fire({
            title: 'تأكيد شراء الأفاتار',
            html: `هل تريد شراء أفاتار <b>${avatarName}</b> لنفسك مقابل <b>${formatNumber(spPrice)} SP</b>؟`,
            imageUrl: btn.closest('.product-card').querySelector('.avatar-product-icon').src,
            imageHeight: 100,
            imageAlt: avatarName,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#5865f2',
            cancelButtonColor: '#d33',
            confirmButtonText: 'نعم، قم بالشراء!',
            cancelButtonText: 'إلغاء'
        }).then(async (result) => {
            if (result.isConfirmed) {
                const originalHTML = btn.innerHTML;
                btn.disabled = true;
                btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span>`;

                try {
                    const data = await apiCall('/api/shop/buy_avatar', {
                        method: 'POST',
                        body: new URLSearchParams({ avatar_id: avatarId })
                    });
                    Swal.fire('تمت العملية!', data.message, 'success');
                } catch (err) {
                    Swal.fire('فشل!', err.message, 'error');
                } finally {
                    btn.disabled = false;
                    btn.innerHTML = originalHTML;
                }
            }
        });
    }

    async function handleAvatarGiftRequest(e) {
        const btn = e.currentTarget;
        const { avatarId, avatarName, spPrice } = btn.dataset;

        if (allCrawlers.length === 0) {
            Swal.fire('خطأ!', 'لا يوجد زواحف لإهدائهم.', 'info');
            return;
        }

        const options = allCrawlers.map(c => `<option value="${c.name}">${c.name}</option>`).join('');
        const { value: selectedCrawler } = await Swal.fire({
            title: `طلب إهداء أفاتار: ${avatarName}`,
            html: `
                <p>اختر الزاحف الذي تريد طلب إهداء هذا الأفاتار له.</p>
                <p>عند موافقة الأدمن، سيتم خصم <b>${formatNumber(spPrice)} SP</b> من رصيدك.</p>
                <select id="crawler-select" class="swal2-select">${options}</select>`,
            imageUrl: btn.closest('.product-card').querySelector('.avatar-product-icon').src,
            imageHeight: 100,
            focusConfirm: false,
            showCancelButton: true,
            confirmButtonText: 'إرسال الطلب للمراجعة',
            cancelButtonText: 'إلغاء',
            preConfirm: () => document.getElementById('crawler-select').value
        });

        if (selectedCrawler) {
            const originalHTML = btn.innerHTML;
            btn.disabled = true;
            btn.innerHTML = `<span class="spinner-border spinner-border-sm"></span>`;
            try {
                const data = await apiCall('/api/gift_request', {
                    method: 'POST',
                    body: new URLSearchParams({
                        avatar_id: avatarId,
                        target_crawler: selectedCrawler
                    })
                });
                Swal.fire('تم إرسال الطلب!', data.message, 'success');
            } catch (err) {
                Swal.fire('فشل!', err.message, 'error');
            } finally {
                btn.disabled = false;
                btn.innerHTML = originalHTML;
            }
        }
    }

    initializeApp();
}
